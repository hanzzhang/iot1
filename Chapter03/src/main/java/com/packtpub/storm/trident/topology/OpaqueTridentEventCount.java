...
public class OpaqueTridentEventCount {
	...
	static EventHubSpoutConfig readEHConfig(String[] args) throws Exception {
		...
		if (args.length > 1) {
			...
			properties.load(OpaqueTridentEventCount.class.getClassLoader().getResourceAsStream("Config.properties"));
		}
	...
	static StormTopology buildTopology(EventHubSpoutConfig spoutConfig) throws Exception {
		...
		state.newValuesStream().each(new Fields(new String[] { "count" }), new OpaqueTridentEventCount.LoggingFilter("Hanz got count: ", 10000));
